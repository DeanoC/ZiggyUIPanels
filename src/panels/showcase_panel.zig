...
// Host-parameterized showcase panel implementation.
//
// This keeps application-level panel behavior in ZiggyUIPanels while allowing
// the host (for example ziggy-ui) to supply concrete rendering/input modules.
pub fn ShowcasePanel(comptime Host: type) type {
    return struct {
        const components = Host.components;
        const draw_context = Host.draw_context;
        const input_router = Host.input_router;
        const input_state = Host.input_state;
        const theme = Host.theme;
        const widgets = Host.widgets;
        const panel_chrome = Host.panel_chrome;
        const theme_runtime = Host.theme_runtime;
        const nav_router = Host.nav_router;
        const surface_chrome = Host.surface_chrome;
        ...
        const showcase_flipbook_manifest = widgets.flipbook.SpriteManifest{
            .atlas_url = "src/panels/assets/panel_frame.png",
